...
func (mysql *MySQL) Save(name, matricula, email, photoPath string) error {
	query := "INSERT INTO alumns (name, matricula, email, photo_url) VALUES (?, ?, ?, ?)"
	result, err := mysql.conn.ExecutePreparedQuery(query, name, matricula, email, photoPath)
	if err != nil {
...
func (mysql *MySQL) ViewAll() ([]entities.Alumn, error) {
	query := "SELECT id, name, matricula, IFNULL(email, ''), IFNULL(photo_url, '') FROM alumns"
	...
	for rows.Next() {
		...
		if err := rows.Scan(&alumn.ID, &alumn.Name, &alumn.Matricula, &alumn.Email, &alumn.PhotoPath); err != nil {
...
func (mysql *MySQL) ViewOne(id int) (*entities.Alumn, error) {
	query := "SELECT id, name, matricula, IFNULL(email, ''), IFNULL(photo_url, '') FROM alumns WHERE id = ?"
...
func (mysql *MySQL) Edit(id int, name, matricula, email, photoPath string) error {
	query := "UPDATE alumns SET name = ?, matricula = ?, email = ?, photo_url = COALESCE(NULLIF(?, ''), photo_url) WHERE id = ?"
	result, err := mysql.conn.ExecutePreparedQuery(query, name, matricula, email, photoPath, id)
	if err != nil {
...
func (mysql *MySQL) UpdatePhoto(id int, photoPath string) error {
	query := "UPDATE alumns SET photo_url = ? WHERE id = ?"
	result, err := mysql.conn.ExecutePreparedQuery(query, photoPath, id)
	if err != nil {
		return fmt.Errorf("error al ejecutar la consulta UPDATE photo: %w", err)
	}

	rowsAffected, _ := result.RowsAffected()
	if rowsAffected == 0 {
		return fmt.Errorf("no se encontró ningún alumno con el ID %d para actualizar foto", id)
	}
	return nil
}
